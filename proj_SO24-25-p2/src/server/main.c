...
static void* session_thread_func(void* arg) {
    ...
    // Envia mensagem de conexão ao cliente (opcional)
    const char* msg_ok = "CONNECTED\n";
    if (write(s->fd_responses, msg_ok, strlen(msg_ok)) < 0) {
        perror("Erro ao enviar mensagem CONNECTED");
    }
